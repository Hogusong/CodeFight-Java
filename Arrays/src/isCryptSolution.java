...
public class isCryptSolution {
    ...
    boolean isCryptSolution(String[] crypt, char[][] solution) {
        double num0 = decryptWord(solution, crypt[0]);
        double num1 = decryptWord(solution, crypt[1]);
        double num2 = decryptWord(solution, crypt[2]);
        return num2 == num0 + num1;
    }

    double decryptWord(char[][] solution, String word){
        String number = "";
        int len = solution.length;
        for(int i=0; i<word.length(); i++){
            for(int j=0; j<len; j++){
                if(word.charAt(i)==solution[j][0]){
                    if(word.length()>1 && i==0 && solution[j][1] == '0')  return -1;
                    number += solution[j][1];
                    j = len;
                }
            }
